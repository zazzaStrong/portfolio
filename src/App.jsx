...
const txt = {
  it: {
    ...
    hi: "// ciao, sono",
    tagline: "PROGETTO.\nCOSTRUISCO.\nDEPLOYO.",
    role: "Full-Stack Developer Â· AI Enthusiast",
    about: "Programmatore full-stack con la passione per l'intelligenza artificiale. Costruisco applicazioni web complete â dal frontend al backend â e integro soluzioni AI per creare prodotti che funzionano davvero.",
    s1: "Linguaggi",
    s2: "Stack",
    s3: "Contatti",
  ...
  en: {
    ...
    tagline: "I DESIGN.\nI BUILD.\nI DEPLOY.",
    role: "Full-Stack Developer Â· AI Enthusiast",
    about: "Full-stack developer with a passion for artificial intelligence. I build complete web applications â from frontend to backend â and integrate AI solutions to create products that actually work.",
    s1: "Languages",
    s2: "Stack",
    s3: "Contact",
...
function MatrixRain() {
  ...
  useEffect(() => {
    ...
    let w, h, cols, drops;
    const chars = "01ã¢ã¤ã¦ã¨ãªã«ã­ã¯ã±ã³ãµã·ã¹ã»ã½ã¿ããããããããããããããããã ã¡ã¢ã¤ã¦ã¨ã©ãªã«ã¬ã­ã¯ã²ã³";
    const resize = () => {
    ...
    <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} style={{
      ...
      {stack.items.map((item) => (
        <div key={item} style={{
          ...
          <span style={{ color: stack.color, opacity: 0.5, marginRight: 8 }}>â</span>{item}
        </div>
      ))}
    </div>
  );
      ...
      <div style={{ background: "#0a0a12", color: "#fff", fontFamily: "'DM Sans', sans-serif", minHeight: "100vh" }}>
        ...
        <div style={{ overflow: "hidden", borderTop: "1px solid rgba(255,255,255,0.03)", borderBottom: "1px solid rgba(255,255,255,0.03)", padding: "16px 0" }}>
          <div style={{ display: "flex", animation: "marquee 25s linear infinite", whiteSpace: "nowrap" }}>
            {Array(5).fill("PYTHON â C â C# â C++ â JAVASCRIPT â HTML â CSS â SQL â FASTAPI â GIT â ").map((s, i) => (
        ...
        <section id="contact" ref={ctaRef} style={{ padding: "100px clamp(32px,8vw,140px) 60px" }}>
          <div style={{
            ...
            >
              ...
              {t.cta}<span style={{ opacity: 0.4 }}>â</span>
            </a>
          </div>
          <div style={{ marginTop: 80, paddingTop: 24, borderTop: "1px solid rgba(255,255,255,0.03)", fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: "rgba(255,255,255,0.08)" }}>Â© 2026 Umberto</div>
        </section>
      </div>
    </>
  );
